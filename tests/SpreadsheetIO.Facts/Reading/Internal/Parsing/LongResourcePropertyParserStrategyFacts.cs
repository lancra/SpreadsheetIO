...
{
    public class LongResourcePropertyParserStrategyFacts
    {
        private readonly AutoMocker _mocker = new AutoMocker();

        private LongResourcePropertyParserStrategy CreateSystemUnderTest()
            => _mocker.CreateInstance<LongResourcePropertyParserStrategy>();

        public class TheTryParseMethod : LongResourcePropertyParserStrategyFacts
        {
            [Theory]
            [InlineData(default)]
            [InlineData("")]
            ...
            {
                // Arrange
                var map = PropertyMapCreator.CreateForFakeParserStrategyModel(model => model.LongNullable);
            ...
            {
                // Arrange
                var map = PropertyMapCreator.CreateForFakeParserStrategyModel(model => model.Long);
                var sut = CreateSystemUnderTest();
            ...
            [Fact]
            public void ReturnsInvalidParseResultWhenCellValueIsNotLong()
            {
                // Arrange
                var map = PropertyMapCreator.CreateForFakeParserStrategyModel(model => model.Long);
                var sut = CreateSystemUnderTest();
            ...
            [Fact]
            public void ReturnsSuccessParseResultWhenCellValueIsLong()
            {
                // Arrange
